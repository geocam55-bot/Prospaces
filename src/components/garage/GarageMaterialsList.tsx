...
import { Box, Hammer, Home, Paintbrush, DoorOpen, SquareStack, Wrench, Zap, Wind, Package } from 'lucide-react';
...
export function GarageMaterialsList({ materials, compact = false }: GarageMaterialsListProps) {
  const categories = [
    ...
    { key: 'framing', label: 'Framing', icon: Hammer, accent: 'text-green-600', card: 'bg-green-50 border-green-200', items: materials.framing || [] },
    { key: 'roofing', label: 'Roofing', icon: Home, accent: 'text-purple-600', card: 'bg-purple-50 border-purple-200', items: materials.roofing || [] },
    { key: 'siding', label: 'Siding & Trim', icon: Paintbrush, accent: 'text-orange-600', card: 'bg-orange-50 border-orange-200', items: materials.siding || [] },
    ...
    ...(materials.insulation ? [{ key: 'insulation', label: 'Insulation', icon: Wind, accent: 'text-teal-600', card: 'bg-teal-50 border-teal-200', items: materials.insulation || [] }] : []),
    ...(materials.drywallAccessories ? [{ key: 'drywallAccessories', label: 'Drywall & Accessories', icon: Package, accent: 'text-slate-600', card: 'bg-slate-50 border-slate-200', items: materials.drywallAccessories || [] }] : []),
  ];
                  ...
                  <tbody>
                    {nonZeroItems.map((item, index) => (
                      <tr key={index} className="border-b border-border hover:bg-muted">
                        ...
                        <td className="py-2 px-3 text-muted-foreground text-sm whitespace-pre-line">{item.notes || '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
